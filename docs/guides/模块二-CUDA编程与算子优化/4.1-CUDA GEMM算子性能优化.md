...
将 quarter-warp 的形状从默认的 $1 \times 8$（行主序）调整为 Z-order 排布，可以让 tileA 和 tileB 的读取**都**触发广播机制，将 memory transaction 从 10 个降至 4 个。
...
<img src="/AIInfraGuide/images/gemm-6.png" alt="方案三：Z-order 线程映射" style="max-width: 80%; display: block; margin: 0 auto;" />
...
> 基线：默认 $1 \times 8$ 行主序排布下 tileA=2 + tileB=8 = **10 transactions**（参见 §8.4）。
> 方案三是 CUTLASS / 主流开源实现的默认选择——以零额外资源开销将 transaction 数压到理论下界。
